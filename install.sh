...
# Skip downloading cyberpanel.sh from network - use local modified version
# This ensures our Ubuntu 24.04 compatibility patches are not overwritten
if [ ! -f "cyberpanel.sh" ]; then
    echo -e "\n❌ Error: cyberpanel.sh not found in current directory!"
    echo -e "Please ensure you are running install.sh from the cyberpanel-2.4.4 directory.\n"
    exit 1
fi
echo -e "\nUsing local modified cyberpanel.sh...\n"

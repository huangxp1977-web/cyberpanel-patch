...
is_package_available() {
    ...
    case "$Server_OS" in
        "CentOS"|"openEuler")
            # For CentOS/openEuler, use yum/dnf list available
            if [[ "$Server_OS_Version" -ge 8 ]]; then
            ...
            ;;
        "Ubuntu")
            # For Ubuntu, use apt-cache policy
            local candidate=$(apt-cache policy "$package" 2>/dev/null | grep 'Candidate:' | awk '{print $2}')
            [[ "$candidate" != "(none)" && -n "$candidate" ]]
            ;;
    esac
}

# Install package only if it's available in the repository
install_package_if_available() {
    ...
    if is_package_available "$package"; then
        ...
        log_warning "Package $package not found in repository, skipping"
    fi
}
...
# Skip git clone - use local modified source code to preserve Ubuntu 24.04 patches
Debug_Log2 "Using local CyberPanel code (Ubuntu 24.04 patched)..,4"
echo -e "\nUsing local CyberPanel source code (Ubuntu 24.04 compatible)...\n"
cp -r . /usr/local/cyberpanel
cd install || exit
...
# Disabled: acme.sh upgrade is not needed after fresh install and causes slow GitHub downloads
...
if [[ "$Server_OS" = "Ubuntu" ]] ; then
  # Install each package individually with availability check
  # This prevents "Unable to locate package" errors for non-existent packages
  log_info "Installing PHP development packages and build tools"
  
  # Core build tools (usually available)
  install_package_if_available "build-essential"
  install_package_if_available "pkg-config"
  install_package_if_available "libmagickwand-dev"
  
  # PHP development packages (may not all be available)
  # lsphp74-dev and lsphp80-dev typically don't exist in Ubuntu repos
  install_package_if_available "lsphp74-dev"
  install_package_if_available "lsphp80-dev"
  install_package_if_available "lsphp81-dev"
  install_package_if_available "lsphp82-dev"
  install_package_if_available "lsphp83-dev"
else

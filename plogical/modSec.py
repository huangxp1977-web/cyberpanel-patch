    ...
    def setupComodoRules():
        try:
            if ProcessUtilities.decideServer() == ProcessUtilities.OLS:
                ...
                rules_dest = "/usr/local/lscp/modsec/rules"
                if not os.path.exists(rules_dest):
                    os.makedirs(rules_dest)

                command = "cp -r coreruleset/rules/* " + rules_dest
                subprocess.call(command, shell=True)

                # Copy setup file
                command = "cp coreruleset/crs-setup.conf.example " + rules_dest + "/crs-setup.conf"
                subprocess.call(command, shell=True)

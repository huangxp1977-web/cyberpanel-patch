...
\n""" % (childDomain, childDomain, childDomain, virtualHostName, virtualHostName, virtualHostName)
                ...
                # Check if Postfix is installed before attempting configuration
                if os.path.exists(postFixPath):
                    ...
                    logging.CyberCPLogFileWriter.writeToFile('[setupAutoDiscover] Postfix not installed, skipping mail server SSL configuration')
...
""" % (virtualHostName, virtualHostName, virtualHostName)
            ...
            if os.path.exists(postFixPath):
                ...
                logging.CyberCPLogFileWriter.writeToFile('[setupAutoDiscover] Postfix not installed, skipping mail server SSL configuration for main domain')
        ...
        try:
            ...
            if os.path.exists(postfixPath):
                try:
                    ...
                    if retValues[0] == 0:
                        logging.CyberCPLogFileWriter.writeToFile(
                            f"[WARNING] DKIM setup failed for {virtualHostName}: {retValues[1]}. "
                            f"Website created but email DKIM may not work. Check Postfix: systemctl status postfix"
                        )
                except Exception as msg:
                    logging.CyberCPLogFileWriter.writeToFile(
                        f"[WARNING] DKIM setup error for {virtualHostName}: {str(msg)}. "
                        f"Website created but email DKIM may not work. Ensure Postfix is installed."
                    )
    ...
    def issueSSLForMailServer(virtualHost, path):
        try:
            ...
            if os.path.exists(filePath):
                ...
                logging.CyberCPLogFileWriter.writeToFile('[issueSSLForMailServer] Postfix not installed, skipping myhostname configuration')

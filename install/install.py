...
class preFlightsChecks:
    ...
    def __init__(self, rootPath, ip, path, cwd, cyberPanelPath, distro, remotemysql=None, mysqlhost=None, mysqldb=None,
        ...
        # Ensure we're in a safe directory to avoid getcwd errors
        try:
            os.chdir('/root')
        except:
            os.chdir('/')
        ...
        if self.distro == ubuntu:
            try:
                ...
                if ubuntu_version.startswith('24.'):
                    # Ubuntu 24.04 - manually add repo
                    self.stdOut("Ubuntu 24.04 detected - adding OpenLiteSpeed repository manually")
                    
                    # Ensure keyrings directory exists
                    os.makedirs("/usr/share/keyrings", exist_ok=True)
                    
                    # Download GPG keys (both old and new for compatibility)
                    # Based on official script: http://rpms.litespeedtech.com/debian/enable_lst_debain_repo.sh
                    # The lst_repo.gpg contains the correct key (011AA62DEDA1F085) for Noble
                    key_path_new = "/usr/share/keyrings/lst_repo.gpg"
                    key_path_old = "/usr/share/keyrings/lst_debian_repo.gpg"
                    
                    # Download new key (RSA 4096, 2016, fingerprint: 011AA62DEDA1F085)
                    command = f"wget --timeout=30 --tries=3 -O {key_path_new} http://rpms.litespeedtech.com/debian/lst_repo.gpg"
                    preFlightsChecks.call(command, self.distro, command, command, 1, 1, os.EX_OSERR)
                    
                    # Download old key (DSA 1024, 2012) for backward compatibility
                    command = f"wget --timeout=30 --tries=3 -O {key_path_old} http://rpms.litespeedtech.com/debian/lst_debian_repo.gpg"
                    preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
                    
                    # Verify the new key file was downloaded successfully
                    if not os.path.exists(key_path_new) or os.path.getsize(key_path_new) == 0:
                        raise Exception(f"Failed to download GPG key: {key_path_new}")
                    
                    # Add repo file with signed-by pointing to the new key
                    repo_content = "# LiteSpeed Tech Repository\n" \
                                   f"deb [signed-by={key_path_new}] http://rpms.litespeedtech.com/debian/ noble main\n"
                    with open('/etc/apt/sources.list.d/lst_debian_repo.list', 'w') as f:
                    ...
                    # Older Ubuntu versions - use script
    ...
    def download_install_CyberPanel(self, mysqlPassword, mysql):
        ...
        # [ANTIGRAVITY FIX] Overwrite downloaded modSec.py with local fixed version
        try:
            # Assume install.py is in <root>/install/install.py
            installer_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            local_modsec = os.path.join(installer_root, 'plogical/modSec.py')
            target_modsec = '/usr/local/CyberCP/plogical/modSec.py'
            
            if os.path.exists(local_modsec):
                logging.InstallLog.writeToFile("Overwriting %s with local fixed version from %s" % (target_modsec, local_modsec))
                shutil.copy(local_modsec, target_modsec)
            else:
                logging.InstallLog.writeToFile("Local modSec.py not found at %s, skipping overwrite" % (local_modsec))
        except Exception as e:
            logging.InstallLog.writeToFile("Failed to overwrite modSec.py: " + str(e))
...
""" % password
    ...
    def fixCyberPanelPermissions(self):
        ...
        # Check if lscpd user exists before proceeding
        # This function is called at the end of installation, but we add this check for safety
        try:
            import pwd
            pwd.getpwnam('lscpd')
        except KeyError:
            # lscpd user doesn't exist, skip all permission fixes
            logging.InstallLog.writeToFile("[fixCyberPanelPermissions] lscpd user not found, skipping permission fixes")
            return

        ###### fix Core CyberPanel permissions

        # Ubuntu uses 'nogroup', CentOS uses 'nobody'
        if self.distro == ubuntu:
            command = "usermod -a -G lscpd,lsadm,nogroup lscpd"
        else:
            command = "usermod -a -G lscpd,lsadm,nobody lscpd"
        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
        ...
        # Only chmod cagefs exclude file if it exists (CloudLinux only)
        ...
        # Only set PowerDNS permissions if PowerDNS is installed
        if self.is_centos_family():
            if os.path.exists('/etc/pdns/pdns.conf'):
            ...
            if os.path.exists('/etc/powerdns/pdns.conf'):
        ...
        # Only chmod log file if it exists
        ...
        # Set proper ownership early - but only if lscpd user exists
        # Check if lscpd user exists before trying to chown
        try:
            import pwd
            pwd.getpwnam('lscpd')
            # User exists, set ownership
            command = "chown -R lscpd:lscpd /usr/local/lscp/cyberpanel/snappymail/"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
        except KeyError:
            # lscpd user doesn't exist yet, skip chown (will be done later)
            logging.InstallLog.writeToFile("[fixCyberPanelPermissions] lscpd user not found, skipping chown (will be set later)")
            pass
        ...
        # Ensure the web server user can access the directories
        # Only run if lscpd user exists
        # Ubuntu uses 'www-data' or 'nobody', CentOS uses 'nobody'
        try:
            import pwd
            pwd.getpwnam('lscpd')
            # Try to add web server user to lscpd group
            # Ubuntu: try www-data first, then nobody
            # CentOS: use nobody
            web_users = ['www-data', 'nobody'] if self.distro == ubuntu else ['nobody']
            for web_user in web_users:
                try:
                    pwd.getpwnam(web_user)
                    command = f"usermod -a -G lscpd {web_user}"
                    preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR, shell=True)
                    break  # Success, no need to try other users
                except KeyError:
                    continue  # User doesn't exist, try next one

            # Fix SnappyMail public directory ownership early
            command = "chown -R lscpd:lscpd /usr/local/CyberCP/public/snappymail/data"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
        except KeyError:
            # lscpd user doesn't exist yet, skip these commands
            logging.InstallLog.writeToFile("[fixCyberPanelPermissions] lscpd user not found, skipping usermod and chown")
            pass
...
"""
            ...
            # Check if lscpd user exists before chown
            try:
                import pwd
                pwd.getpwnam('lscpd')
                ...
                # lscpd user doesn't exist, skip chown
                logging.InstallLog.writeToFile("[download_install_phpmyadmin] lscpd user not found, skipping chown")
                pass
    ...
    def downoad_and_install_raindloop(self):
        try:
            ...
            # Set proper ownership and permissions for SnappyMail - only if lscpd user exists
            try:
                import pwd
                pwd.getpwnam('lscpd')
                ...
                # lscpd user doesn't exist yet, skip these commands
                logging.InstallLog.writeToFile("[downoad_and_install_raindloop] lscpd user not found, skipping chown and usermod")
                pass
...
#             preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
    ...
    def findSSHPort(self):
        try:
            ...
            for items in sshData:
                ...
                items = items.strip()
                if not items or items.startswith('#'):
                    continue
                    
                # Look for Port directive
                if items.startswith('Port '):
                    port = items.split()[1]
    ...
    def installLSCPD(self):
        try:
            ...
            # Clean up any existing lscpd user/group
            command = 'userdel -r lscpd 2>/dev/null || true'
            subprocess.call(command, shell=True)
            command = 'groupdel lscpd 2>/dev/null || true'
            subprocess.call(command, shell=True)
            os.chdir(self.cwd)
    ...
    def setupComodoRules(self):
        try:
            ...
            # Install OWASP ModSecurity Core Rule Set (CRS) instead of broken Comodo rules
            if os.path.exists('coreruleset'):
                import shutil
                shutil.rmtree('coreruleset', ignore_errors=True)

            # Clone OWASP CRS
            command = "git clone https://github.com/coreruleset/coreruleset.git"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            # Copy rules to destination
            command = "cp -r coreruleset/rules/* /usr/local/lscp/modsec/rules/"
            # Ensure destination directory exists first
            if not os.path.exists("/usr/local/lscp/modsec/rules"):
                os.makedirs("/usr/local/lscp/modsec/rules")
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)

            # Copy setup file
            command = "cp coreruleset/crs-setup.conf.example /usr/local/lscp/modsec/rules/crs-setup.conf"
            preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
        ...
        """
            ...
            try:
                ...
                # lscpd user doesn't exist, skip chown
                logging.InstallLog.writeToFile("[setupComodoRules] lscpd user not found, skipping chown")
                pass
...
"""
    ...
    def setupPHPSymlink(self):
        try:
            ...
            if not os.path.exists('/usr/local/lsws/lsphp82/bin/php'):
                ...
                # Install PHP 8.2 (check availability first)
                if self.distro == centos or self.distro == cent8 or self.distro == openeuler:
                    # Check yum availability
                    check_cmd = 'yum list available lsphp82 2>/dev/null | grep -q lsphp82'
                    if subprocess.call(check_cmd, shell=True) == 0:
                        command = 'yum install lsphp82 -y'
                        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR, shell=True)
                    else:
                        logging.InstallLog.writeToFile("[setupPHPSymlink] PHP 8.2 not available in repository, skipping")
                else:
                    # Check apt availability
                    check_cmd = 'apt-cache search ^lsphp82$ 2>/dev/null | grep -q "^lsphp82 "'
                    if subprocess.call(check_cmd, shell=True) == 0:
            ...
            # Install PHP 8.4 (check availability first)
            if not os.path.exists('/usr/local/lsws/lsphp84/bin/php'):
                logging.InstallLog.writeToFile("[setupPHPSymlink] PHP 8.4 not found, checking availability...")
                
                if self.distro == centos or self.distro == cent8 or self.distro == openeuler:
                    # Check if package is available in yum
                    check_cmd = 'yum list available lsphp84 2>/dev/null | grep -q lsphp84'
                    if subprocess.call(check_cmd, shell=True) == 0:
                        command = 'yum install lsphp84 lsphp84-* -y'
                        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR, shell=True)
                    else:
                        logging.InstallLog.writeToFile("[setupPHPSymlink] PHP 8.4 not available in repository, skipping")
                else:
                    # Check if package is available in apt
                    check_cmd = 'apt-cache search lsphp84 2>/dev/null | grep -q "^lsphp84 "'
                    if subprocess.call(check_cmd, shell=True) == 0:
                        command = 'DEBIAN_FRONTEND=noninteractive apt-get -y install lsphp84 lsphp84-*'
                        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR, shell=True)
                    else:
                        logging.InstallLog.writeToFile("[setupPHPSymlink] PHP 8.4 not available in repository, skipping")

            # Install PHP 8.5 (check availability first)
            if not os.path.exists('/usr/local/lsws/lsphp85/bin/php'):
                logging.InstallLog.writeToFile("[setupPHPSymlink] PHP 8.5 not found, checking availability...")
                
                if self.distro == centos or self.distro == cent8 or self.distro == openeuler:
                    # Check if package is available in yum
                    check_cmd = 'yum list available lsphp85 2>/dev/null | grep -q lsphp85'
                    if subprocess.call(check_cmd, shell=True) == 0:
                        command = 'yum install lsphp85 lsphp85-* -y'
                        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR, shell=True)
                    else:
                        logging.InstallLog.writeToFile("[setupPHPSymlink] PHP 8.5 not available in repository, skipping")
                else:
                    # Check if package is available in apt
                    check_cmd = 'apt-cache search lsphp85 2>/dev/null | grep -q "^lsphp85 "'
                    if subprocess.call(check_cmd, shell=True) == 0:
                        command = 'DEBIAN_FRONTEND=noninteractive apt-get -y install lsphp85 lsphp85-*'
                        preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR, shell=True)
                    else:
                        logging.InstallLog.writeToFile("[setupPHPSymlink] PHP 8.5 not available in repository, skipping")
    ...
    def installOne(package):
        # Use shell=True to properly handle environment variables like DEBIAN_FRONTEND
    ...
    def installRestic(self):
        try:
            ...
            if os.path.exists(CentOSPath) or os.path.exists(openEulerPath):
                ...
                preFlightsChecks.call(command, self.distro, command, command, 1, 0, os.EX_OSERR)
                # restic self-update command was removed in newer versions of restic
...
"""
    ...
    def installAcme(self):
        # Directly download and install acme.sh with timeout to avoid hanging
        # Try GitHub first with timeout, fallback to Gitee mirror if it fails
        command = '''
        cd /tmp && \
        (wget --timeout=60 --tries=2 -O acme.sh.tar.gz https://github.com/acmesh-official/acme.sh/archive/master.tar.gz || \
         wget --timeout=60 --tries=3 -O acme.sh.tar.gz https://gitee.com/neilpang/acme.sh/repository/archive/master.tar.gz) && \
        tar -xzf acme.sh.tar.gz && \
        cd acme.sh-master && \
        ./acme.sh --install --home /root/.acme.sh --config-home /root/.acme.sh/data && \
        cd /tmp && rm -rf acme.sh.tar.gz acme.sh-master || true
        '''
        subprocess.call(command, shell=True)
...
?>""" % (generate_pass())
        ...
        # Ensure web server users are in the lscpd group - only if lscpd exists
        # Ubuntu: try www-data first, then nobody
        # CentOS: use nobody
        try:
            import pwd
            pwd.getpwnam('lscpd')
            # Try to add web server user to lscpd group
            web_users = ['www-data', 'nobody'] if ubuntu else ['nobody']
            for web_user in web_users:
                try:
                    pwd.getpwnam(web_user)
                    command = f"usermod -a -G lscpd {web_user}"
            ...
            # lscpd user doesn't exist, skip
            logging.InstallLog.writeToFile("[main] lscpd user not found, skipping usermod")
            pass
        ...
        try:
            ...
            logging.InstallLog.writeToFile("[main] lscpd user not found, skipping chown")
            pass
    except:
        pass
